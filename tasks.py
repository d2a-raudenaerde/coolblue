...
def coolblue_voorbeeld():
    ...
    """
    browser.configure( browser_engine="chromium", screenshot="only-on-failure", 
                       headless=False,)
    try:
        page = browser.page()
        ...
        # Locator voor het "query" <input> element  
        search = page.locator("input[name='query']")

        # Wacht (max 3 sec) tot het zichtbaar is
